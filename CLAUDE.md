...
- **下一跳（runner 到手前软件已合）**：NOW-06 已并入本线。3101 **已在 RK3528 现网**（MagicDNS `http://rk3528.tail6d8b09.ts.net:3101/`；`real-readonly`/`observer`）。盒上 dsh 只许 `maintain.sh` 四动词。**不做** 3101 聊天框连 worker dsh。NAS 共享布局：`/mnt/nas/.bogda/inbox` + `/mnt/nas/.bogda/runner/artifacts`（`prepare_runner_share.sh`）。owner 接 runner 后按 [`2026-09-02-bogda-runner-handshake.md`](docs/reports/2026-09-02-bogda-runner-handshake.md) 填精确白名单与 HMAC。**仍缺** DEF-03 真 checkpoint。**不要**接 Wake Bridge、**不要**改 3100、**不要**把研究任务丢进 `pi-service`。
