...
- bogda Gate 6：**通过**（2026-08-31）。Gate 7：**S1/S2 影子通过**。NOW-06 软件已合。3101 **已在 RK3528**：MagicDNS `http://rk3528.tail6d8b09.ts.net:3101/`（只读首发）。NAS inbox `/mnt/nas/.bogda/inbox`。**不做** 3101 聊天框连 worker dsh。接 runner 见握手说明。**不要**切 3100、**不要**研究进 `pi-service`
...
- 雷达→Zotero：9.8 后再设计
- 控制台：Orchestra 3100 仍在本机；Bogda 3101 在盒子 MagicDNS `:3101`。v1.5 3100 手机访问仍挂账
