...
Event storage is caller-selected, for example `JsonlRunEventSink(Path("events.jsonl"))`. The sink uses a process-local lock, append mode, flush/fsync, and exact validated lines. It does not provide inter-process locking or durable cross-process exactly-once delivery; service replay/idempotence tracking is also process-local. No production writer is wired by Phase B.
