...
- **角色退役了，硬编码的界面文案不会自己退役**：3101 早在 DEF-03 就接替了 3100 成为生产控制台，但页脚一直写着"3101 影子运行 / 影子控制台 · 不写现网 Prefect"——它是 `AppShell.tsx` 里的一行死字符串，后端能力早就全绿了。规则：改角色/改拓扑时把"界面自称"当验收项之一；状态类文案应从 capabilities 派生（profile/role/canSubmit…），不要写死在组件里。
