...
| NOW-06 | 正式研究运行前接线 | Gate 7 审查 Important 已修。本地已做身份/日志 API/产物清理内核/>20 CNY HMAC 凭证/usage-unknown 适配器/峰谷 dispatcher。未接现网 worker、未签发 UI、未切 3100。S2 验收资源仍留 Prefect | [本轮报告](2026-09-02-bogda-now06-pre-research.md)；**不**接宿舍机、**不**切 3100 |
...
| DEF-04 | usage unknown 的真实接线闭环：把 core 持久恢复状态映射到 real profile 的 owner API/窗口 | **本地 adapter 已把 SQLite case 映到既有 reconcile/retry/terminate**（需 `BOGDA_USAGE_UNKNOWN_DB`）。尚未接到现网 worker 的同一 DB | 第一条需要通过 real profile 人工恢复的 provider 调用前 | 决策中心已有“先对账、确认重试、终止”窗口；真实接线不得改变字段、revision 或禁止同 call 重试语义 | 本地证据见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；现网同一文件仍待 |
...
| DEF-06 | 将已验证的峰谷计划器接入真实 scheduler/dispatcher，并提供“继续等/按高峰新预算立即跑/取消” | **本地 `PriceAwareDispatcher` 三选一已测，不调用 Prefect。** 生产调度执行尚未接线 | real profile 开始自动安排付费运行前 | 创建/确认窗口需显示计划启动、当前价格窗口、预计节省、deadline 风险和 owner 三选一 | 本地证据见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；现网 dispatcher 仍待 |
...
| DEF-13 | 运行产物生命周期：prompt/stdout/stderr/receipt 的权限、保留期、清理与科研可复现引用 | **本地 `ArtifactLifecycle` 显式 `delete-content` 已测**（tombstone + 保留 events）。3101 尚未放清理按钮 | 正式产生真实科研产物前 | 运行详情提供可点击引用和权限状态；清理必须是显式受控动作 | 内核见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；console 清理面仍待 |
...
| DEF-16 | 超过 20 CNY 的预算在 owner 批准后形成不可伪造、可重放审计的核心审批凭证，并允许同一冻结 envelope 继续准入 | **HMAC 单次凭证 + `admit(..., approval=)` 已测**（伪造/过期/错 run/重放拒绝）。决策中心尚未签发 UI | 第一次需要执行 `>20 CNY` 的真实运行前 | 决策中心展示 envelope、模型、峰谷、余额和批准上限；批准后生成绑定 run/pricing/revision 的凭证 | 内核见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；签发 UI 仍待 |
| DEF-17 | 运行详情可读取受控的原始 stdout/stderr/结构化运行日志，而不只显示 metadata URI | **`GET /api/v1/runs/{id}/logs` + 运行详情日志区已落地**（路径封闭、脱敏、截断）。现网需 `BOGDA_ARTIFACT_ROOT` | 真实 worker 产物根配置后做只读抽查 | 运行详情提供按来源筛选、截断提示；秘密脱敏且不得任意读宿主路径 | 本地见 [NOW-06](2026-09-02-bogda-now06-pre-research.md) |
...
| DEF-19 | 对真实 profile 建立身份与角色边界，区分 owner、只读观察者和运维管理员 | **本机 env 角色已落地**（observer 即使有 allowlist 也不能 POST）。不是 OIDC/多用户会话 | 3101 对多用户或非本机开放前 | 前端显示当前身份/角色；每个动作同时由服务端授权，不能只靠禁用按钮 | 本地越权测试见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；非本机 RBAC 仍待 |
