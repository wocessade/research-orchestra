...
| DEF-20 | 将 capability 细化到资源/动作适用性，并为 checkpoint 引入独立 `canDecideCheckpoint` | 当前全局 capability 与资源 allowlist 可能使按钮先可用、提交后才 403；checkpoint 还借用了科研评审能力 | real profile 开放任一写动作前 | 每个按钮显示可执行、只读或不适用的明确原因；不可把前端禁用当授权 | Stage E/切换前；资源级 allowlist、后端授权与前端状态矩阵一致，checkpoint 不依赖 review capability |
