...
| DEF-13 | 运行产物生命周期：prompt/stdout/stderr/receipt 的权限、保留期、清理与科研可复现引用 | **`POST .../artifacts/cleanup` + 运行详情按钮已落地**（确认词 `delete-content`，events 保留）。现网需 `BOGDA_ARTIFACT_ROOT` 与 worker 同一根 | 正式产生真实科研产物前 | 运行详情提供可点击引用和权限状态；清理必须是显式受控动作 | 见 [NOW-06](2026-09-02-bogda-now06-pre-research.md)；现网抽查仍待 |
...
| DEF-16 | 超过 20 CNY 的预算在 owner 批准后形成不可伪造、可重放审计的核心审批凭证，并允许同一冻结 envelope 继续准入 | **签发 API/UI + 共享库 `get_open`/`consume_open` 已落地**（MAC 不回显）。worker 需同一 `BOGDA_APPROVAL_DB` 与 HMAC | 第一次需要执行 `>20 CNY` 的真实运行前 | 运行预算区签发绑定 envelope 的凭证；伪造/过期/错 run/重放拒绝 | 见 [NOW-06](2026-09-02-bogda-now06-pre-research.md) 与 [handshake](2026-09-02-bogda-runner-handshake.md) |
