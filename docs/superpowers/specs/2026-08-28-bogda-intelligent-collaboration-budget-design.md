...
前端必须让 owner 看见 Bogda 为什么等待、准备花多少钱、准备采用什么模型，以及当前究竟需要人做什么。需要裁决的事项进入统一入口，不能只藏在运行日志或单个任务详情中。

现有“科研自主模式”面板继续只负责 autonomy，不把模型、预算和任务意图塞进同一个控件。常用操作直接可见，高级估算参数渐进展开；系统默认值必须能安全运行，不能要求用户每次填写完整预算表。

### 13.2 必需的操作面

| 操作面 | 用途 | 关键内容 |
|---|---|---|
| 决策中心 | 聚合所有等待 owner 的事项 | 待处理数量、类型、项目、阻塞时长、风险、预计费用和截止时间 |
| 创建运行窗口 | 冻结本次合作与预算策略 | `intent`、模型档位、预计工作量、deadline、错峰偏好和预算摘要 |
| 启动前确认窗口 | 展示系统最终解析结果 | 有效 autonomy、实际模型、预计费用、授权上限、计划启动时间和降级策略 |
| 运行详情 | 观察并干预单次运行 | 已用/预留/剩余预算、模型调用、暂停原因、恢复条件、事件日志和 prompt 产物 |
| 项目策略页 | 设置项目默认值 | autonomy 覆盖、默认模型路由、预算策略、错峰策略和通知偏好 |
| 全局策略页 | 设置跨项目安全基线 | 最低余额、快照时效、默认预算、安全余量、全局单飞锁和价格目录状态 |

决策中心是唯一统一待办入口。首页、项目页和运行详情可以显示同一事项的入口或徽标，但不能各自维护一份待办状态。

### 13.3 必须由 owner 显式裁决的事项

| 触发场景 | 前端必须提供的选项 |
|---|---|
| 研究计划需要批准 | 批准、退回修改、终止；可填写理由 |
| 科研结果需要裁决 | `accepted`、`rejected`、`inconclusive`；必须看到证据与产物引用 |
| 运行请求提高预算 | 批准新上限、维持原预算继续等待、停止运行 |
| `decide`/`audit` 无法使用 Pro | 等待 Pro、明确批准改用 Flash、终止 |
| 非紧急任务准备按高峰价立即执行 | 继续等待空闲、按新预算立即运行、取消 |
| 调用开始但费用结果未知 | 先对账、确认重试、终止；不得提供无提示的一键重试 |
| 对外发布、采购、新增支出或超预算实验 | 批准或拒绝；autonomous 也不能跳过 |

每个裁决窗口必须同时显示：触发原因、可选动作、每个动作的费用或质量影响、相关证据、不可逆后果和将写入日志的决定摘要。需要扩大权限或预算的确认不得使用预选中的危险选项。

### 13.4 无需逐次打扰 owner 的自动行为

以下行为在已冻结策略内自动执行，并通过通知或运行日志可见：

- 在预算包内从 Flash 升级 Pro；
- execute/brief/explore 按已批准策略降级；
- 在 deadline 内等待空闲时段；
- 余额恢复且无需扩大预算时自动恢复；
- 基础设施步骤按声明规则重试；
- 保存 checkpoint、必要产物和结构化日志。

自动行为一旦需要突破预算、降低 decide/audit 质量或进入 Type-B 科学与外部决策，立即转成决策中心待办。

### 13.5 显式开关与选择器

| 控件 | 作用域 | 建议默认值 |
|---|---|---|
| autonomy：手动/监督执行/范围内自主 | 全局默认 + 项目覆盖 | `supervised` |
| 模型档位：Auto/Flash/Pro | 项目默认 + 单次运行 | `auto` |
| 允许预算内自动升级 Pro | 项目 + 单次运行 | 开 |
| 允许低风险 intent 降级 Flash | 项目 + 单次运行 | 开；仅 execute/brief/explore |
| 优先空闲时段 | 项目 + 单次运行 | 开 |
| 余额恢复后自动继续 | 项目 + 单次运行 | 开；预算包不变时有效 |
| 关键待办通知 | 全局 + 项目 | 开 |
| 项目继承/覆盖全局策略 | 项目 | 继承 |

所有项目覆盖都必须显示当前值来源、修订号和“恢复继承”入口。单次运行覆盖只影响本次运行，并在确认窗口中突出显示。

### 13.6 不允许做成普通关闭开关的安全约束

以下约束可以在专门策略页调整合法参数，但不能在单次运行中关闭或绕过：

- 用量快照过期时 fail-closed；
- 账户最低保留余额；
- decide/audit 的静默降级禁令；
- 科研结论、对外发布、采购和新增支出的人工裁决；
- `model_call_started` 后费用未知时的恢复门禁；
- 结构化事件日志、prompt 归档和秘密脱敏；
- 预算提高必须形成显式审批事件。

### 13.7 状态、窗口与可操作性

预算不足、价格目录失效、用量快照过期、等待空闲时段、等待科学裁决和费用未知必须使用不同状态名称、图标和处理按钮，不能统一显示为“模型不可用”或“任务暂停”。

阻塞事项使用对话框或侧边抽屉处理；普通状态变化不得频繁弹窗打断用户。决策中心按“需要我现在处理”“有截止时间”“仅供知晓”分组，并支持按项目和风险筛选。

所有写操作使用后端修订号做冲突检测。发生冲突时展示最新状态和差异，要求用户重新确认，不能静默覆盖。关键操作完成后提供可点击的运行、事件和产物引用。
...
- 决策中心聚合预算、质量、科学判断和外部动作待办，不产生重复事实源。
- decide/audit 降级、提高预算、费用未知重试和 Type-B 操作必须经过显式确认。
- 安全约束不能通过单次运行开关绕过。
- 项目覆盖显示来源、修订号，并能恢复继承。
- 冲突响应展示最新状态并要求重新确认。
...
- owner 可以在统一决策中心处理所有阻塞事项，并在创建、启动和运行详情中看见模型、预算、价格窗口和恢复条件。
- 所有扩大预算、降低 decide/audit 质量、科学裁决和外部动作都有显式窗口及结构化审批事件。
