...
class TestInjectDaily(unittest.TestCase):
    def setUp(self):
        ...
        self.env.update({
            ...
            # as_posix：脚本内 sed 替换用这些路径做 RHS，Windows 反斜杠会被
            # sed 当转义（\1 组引用报错）；Git Bash 接受正斜杠路径
            "ORCHESTRA_TASKS_DIR": self.tasks.as_posix(),
            "ORCHESTRA_RESULTS_DIR": self.results.as_posix(),
    ...
    def test_injects_four_ordered_tasks_with_dependencies(self):
        ...
        self.assertIn(self.results.as_posix(), files[-1].read_text(encoding="utf-8"))
    ...
    @unittest.skipUnless(
        os.name == "posix",
        "Git Bash 的 kill -0 无法探测原生 Windows PID（msys 限制），该回退场景只在 POSIX 可测",
    )
    ...
    def test_same_date_injections_are_mutually_exclusive(self):
        ...
        slow_bin.mkdir()
        # Windows 下 which 返回反斜杠路径，写进 sh 脚本会被当转义吞掉 → 转正斜杠
        real_sed = (shutil.which("sed") or "/bin/sed").replace("\\", "/")
