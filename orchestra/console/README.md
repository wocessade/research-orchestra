...
| startup_serve.bat / startup_homepage.bat | 开机自启项（复制于用户启动文件夹，下次登录生效） |
| out/ | 产物（不入库） |
| tests/ | unittest（`python -m unittest discover tests -v`，69 个） |

## 使用

- 手动刷新：`python console_feed.py refresh`（自动档：计划任务 OrchestraConsoleRefresh 每 10 分钟）
- 手动起服务：`python console_feed.py serve`
- 开机自启：refresh 走计划任务；serve 与 Homepage 走用户启动文件夹（startup_*.bat，非提权 shell 注册 onlogon 计划任务被拒后的落地方式，2026-08-20 用户拍板）
