...
> **~18 KB reference file.** Prefer on-demand partial reads.
> **Sections:** `## 背景` · `## 验证五维` · `## 三条验证路径` · `### Tier 1：OpenAlex / Crossref 验证（有 DOI 的文献）` · `### Tier 2：CNKI 交叉验证（无 DOI 或有 DOI 但 OpenAlex 查不到）` · `### Tier 3：结构化人工验证（无 DOI 且无 CNKI 记录）` · `## 验证路径选择流程图` · `## 验证状态标记规范` · `## 按文献类型的验证路径速查` · `## 验证流程` · `### 第一阶段：入库验证（S2 文献调研时）` · `### 第二阶段：出库复核（S3-S4 写作时 + S6 引文验证时）` · `### 第三阶段：审查前全面审计（S7 / T5 / T6）` · `## 批量验证脚本`
> `### 输入格式` · `### 输出 verdict 三态判决逻辑` · `## 污染信号 Schema（Contamination Signals）` · `### Schema` · `### 预印本域名列表（PREPRINT_VENUES）` · `### 污染层级计算（k/k_max）` · `### 与现有验证状态标记的关系` · `## 常见问题处理` · `## 门控集成` · `### S2 Gate (Q2)` · `### S6 Gate (Q6)` · `### S7 Gate (Q7)` · `### T5/QT5 + T6 审查看板` · `## 中文文献验证的实操建议`

AI 辅助写作中最危险的故障模式是**编造参考文献** — DOI 看着像真的但不解析、作者名与实际不符、出处张冠李戴。评审人一眼就能看出这类问题，一旦发现会严重损害论文的可信度。

本文档覆盖从文献入库到终稿定稿的**全生命周期验证**，整合到已有的 S2 文献调研、S6 引文验证、S7 润色审查、T5/T6 论文审查/修订四个阶段中。
...
1. **用户提供 CNKI 导出文件**（T2B/T5 步骤中要求用户导出）
...
`references/playwright-browser-automation.md` 中的 DOI 浏览器验证流程是推荐的手动验证方法。功能：
- **逐 DOI 浏览器验证**：用 Playwright 在浏览器中打开 resolve 页面，确认文章存在且元数据匹配
- **三态判决**：`true`（匹配）、`false`（DOI 级不匹配=造假证据）、`unresolvable`（仅标题搜索失败=覆盖缺口）
- **豁免**：`source: manual` 的条目跳过自动验证

批量验证目前使用手动方式，通过 Playwright MCP 自动化打开验证链接。见 `references/playwright-browser-automation.md` §5 (DOI 浏览器验证批量自动化)。
...
```
☐ 文献审计已完成 — 无 Critical 问题
☐ 全部引用已有确定验证状态（✅ 或 ⚠️ 用户已确认）
☐ CCF 标注检查完成（仅 STEM 学科）
☐ 重复引用已排查
```
...
3. **最可靠的中文验证方式仍然是 CNKI 导出。** 在 S2/T2 阶段主动要求用户提供 CNKI 搜索结果导出文件，这是性价比最高的方式。
