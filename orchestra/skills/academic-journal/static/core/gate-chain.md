...
**Thesis:**
```
QT1 → QT1.5* → QT2 → QT3 → QT4 → QT5 → QT6 → [T6.7† → T5 → T6 re-review] → QT6.5** → QT7
```
...
| Q7 | BLOCK | `skills-embedded/paper-audit.md` gate returns PASS. No Critical or major items remain. Moderate/minor advisory items may persist. Factual accuracy check (S7G) complete — no Critical factual errors; all Major factual errors resolved under convergence-loop.md; accepting a scope limitation cannot waive missing evidence. **P1:** Major prose rewrites logged in `.paper/rewrite_matrix.md` with Status=done (or waived). |
...
| QT6 | SOFT BLOCK | Follow convergence-loop.md: REVIEW_STABLE is separate from READY. Zero open Critical/Major in the cumulative register, no regression, and applicable evidence checks on the current manuscript. Unmatched old issues require closure evidence. At the three-round cap, return unfinished draft; do not pass or auto-advance. Major rewrites remain recorded in rewrite_matrix.md. |
| QT6.7 | COND. BLOCK | User-requested gap expansion with actual evidence. Update ledger/citation bank and re-run T5 then T6 on the revised manuscript. Pass only when READY; mark expansion_recheck_complete for that version. Repeat T6.5 for master on the reviewed version before requested defense. No automatic heavy expansion or word-count-only pass. |
