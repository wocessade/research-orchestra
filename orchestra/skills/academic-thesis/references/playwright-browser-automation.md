...
标准的 CNKI 搜索流程，适用于 T2C 等所有需要中文学术文献检索的阶段。
...
从 `search_protocol.md`（T2A）读取关键词组，依次搜索：
...
用于论文 snowballing（T2），补充 API snowballing 的覆盖不足。
...
4. **交叉验证：** 浏览器提取的结果仍需要通过 T2 可信度筛选—— 不会因为流程自动化就降低验证标准。
...
**Trigger:** Run after CNKI search completes (T2C). Apply to the top-5 most relevant or most-cited Chinese papers.
...
- Discard any paper already present in the primary CNKI search results (T2C output)
