#!/usr/bin/env python3
"""
Validate the academic-thesis skill for cross-file consistency.
...
# Stale patterns were previously defined here and have all been resolved.
# Keeping the block for future use when new stale patterns are identified.
stale_patterns = []
...
SHARED_AGENTS_DIR = SKILL_DIR / ".." / "academic-shared" / "evaluate" / "agents"
...
def check_agent_file(agent_id: str, tier: str, config_name: str):
    ...
    if tier in ("humanities_bachelor",):
        ...
        label = f"evaluate/agents/humanities/{agent_id}.md"
    elif tier in ("stem_bachelor",):
        agent_path = STEM_DIR / f"{agent_id}.md"
        fallback = AGENTS_DIR / f"{agent_id}.md"
        label = f"evaluate/agents/stem/{agent_id}.md"
    else:
        # master tier: agents live in academic-shared/evaluate/agents/
        agent_path = SHARED_AGENTS_DIR / f"{agent_id}.md"
        fallback = AGENTS_DIR / f"{agent_id}.md"
        label = f"../academic-shared/evaluate/agents/{agent_id}.md"
    if not agent_path.exists():
        if not fallback.exists():
            err(f"Config '{config_name}': agent '{agent_id}' not found at {label} (or evaluate/agents/{agent_id}.md)")
